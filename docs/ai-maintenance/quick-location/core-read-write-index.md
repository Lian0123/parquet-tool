...
`Schema.create` and `Schema.addColumn` require at least one column. `ParquetWriter.write` buffers rows and flushes row groups; `close()` finalizes the file. `openForAppend` reuses the file schema and adds row groups. `ParquetReader` exposes metadata, row groups, all rows, and an iterator; a closed reader cannot be used.
