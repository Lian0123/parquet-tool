...
| CI | Linux, macOS, and Windows build/test; lint runs on Linux; release builds three prebuild targets | `.github/workflows/` |
...
Readers and writers require explicit `close()`. Writer close flushes the final row group. Append adds row groups; it does not rewrite existing rows. `parallel*` uses Promise-based buckets and multiple reader handles, not worker threads or separate processes.

## Support boundaries

- Scalars are `boolean | number | bigint | string | null`.
- `Schema.parseType` supports BOOLEAN, INT32, INT64, FLOAT, DOUBLE, and BYTE_ARRAY. Enum members alone do not prove end-to-end support.
